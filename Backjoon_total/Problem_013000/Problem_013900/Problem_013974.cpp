...
int main(void) {
    ...
    while(T--) {
        ...
        for(int m=2; m<=K; m++) {
            for(int i=0; i+m<=K; i++) {
                dp[i][i+m]=(long long)INT_MAX;
                for(int j=check[i][i+m-1]; j<=check[i+1][i+m]; j++) {
                    if(j<=i)
                        continue;
                    if(j>=i+m)
                        break;
                    
                    if(dp[i][i+m]>dp[i][j]+dp[j][i+m]+sum[i+m]-sum[i]) {
                        dp[i][i+m]=dp[i][j]+dp[j][i+m]+sum[i+m]-sum[i];
                        check[i][i+m]=j;
                    }
                }
            }
